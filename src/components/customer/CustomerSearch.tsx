...
export default function CustomerSearch() {
  const [searchType, setSearchType] = useState('CIF');
  ...
  return (
    <div className="p-8 h-full overflow-y-auto flex flex-col gap-6">
      <header className="flex justify-between items-end bg-white p-6 rounded border border-slate-200 shadow-sm">
        ...
        <button className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-[11px] font-bold rounded shadow-md shadow-blue-600/20 hover:bg-blue-700 transition-all active:scale-95 uppercase tracking-widest">
          <Plus size={14} />
          Create New Entity
        </button>
      </header>
      ...
      <div className="bg-white rounded-lg border border-bank-border shadow-sm overflow-hidden">
        ...
        <div className="p-4 bg-slate-50 border-t border-bank-border flex justify-end gap-3">
          <button className="px-6 py-2 text-xs font-bold text-slate-500 hover:text-bank-text-primary uppercase">Clear Form</button>
          <button className="px-8 py-2 bg-bank-sidebar text-white text-xs font-bold rounded shadow hover:bg-bank-active transition-all uppercase tracking-widest flex items-center gap-2">
            Process Inquiry
            <ArrowRight size={14} />
          </button>
        </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-0">
        <div className="p-4 bg-slate-50 border-b border-bank-border flex justify-between items-center">
          <span className="text-xs font-bold uppercase tracking-tight">Search Results [4 Records Found]</span>
        ...
        <div className="flex-1 overflow-auto">
          <table className="finacle-grid">
            ...
            <tbody className="text-xs text-slate-600">
              {[
                { id: '99018273', name: 'ADITI RAO HYDARI', type: 'RETAIL', status: 'ACTIVE', phone: '+91 99283-0012' },
                { id: '99018442', name: 'LIAM NEESON', type: 'RETAIL', status: 'ACTIVE', phone: '+44 20-7946-0128' },
                { id: '11029384', name: 'AURELIUS CONSTRUCTIONS LTD', type: 'CORPORATE', status: 'ACTIVE', phone: '+39 06-1234-5678' },
                { id: '88273645', name: 'MARCUS AURELIUS', type: 'RETAIL', status: 'INACTIVE', phone: '+39 06-9876-5432' },
              ].map((cust) => (
                <tr 
                  key={cust.id} 
                  className="hover:bg-slate-50 border-b border-slate-100 transition-colors group cursor-pointer"
                  onClick={() => window.dispatchEvent(new CustomEvent('nav-to-customer', { detail: cust }))}
                >
                  <td className="font-mono text-blue-600 font-bold group-hover:underline">{cust.id}</td>
                  <td className="font-bold text-slate-700 uppercase">{cust.name}</td>
                  ...
                  <td className="font-mono font-medium text-slate-500">{cust.phone}</td>
                  <td>
        ...
        <div className="p-3 bg-slate-50 border-t border-slate-100 flex justify-between items-center text-[10px] text-slate-500 font-bold uppercase tracking-tight">
          <div>Showing 4 of 4 records matched</div>
